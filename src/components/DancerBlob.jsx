 ...
 * dancer on top.
 *
 * The caller sizes and places it through `className` (the box is always
 * 11:10). `media` is the viewport where it is shown: the dancer's chunk is
 * fetched only there and once the blob comes near the viewport, so the other
 * layout never downloads it. `areaRef` widens where the jelly listens for
 * mouse/touch; by default it is the blob's own box.
 */
export default function DancerBlob({ wordmark, media, areaRef, className = '' }) {
  ...
  return (
    <div ref={boxRef} aria-hidden="true" className={`dancer-blob aspect-[11/10] ${className}`.trim()}>
      <svg
      ...
      {/* 0.24 × the blob's height (the box is its own size container): both
          words sit on the solid part. */}
      <p className="pointer-events-none absolute inset-0 flex select-none flex-col items-center justify-center font-display text-[24cqh] font-extrabold leading-[0.85] tracking-tight text-ink/[0.07]">
        {wordmark.map((word) => (
          <span key={word}>{word}</span>
        ))}
      </p>

      {/* The Lottie canvas (612 × 736): 79.4 % of the blob's height, centred. */}
