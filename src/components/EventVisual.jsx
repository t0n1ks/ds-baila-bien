  ...
  return (
    <div ref={ref} className="event-column absolute inset-0">
      <div className="event-block flex h-full flex-col">
        {/* Centred over the blob, on tablet and desktop alike. */}
        <h3 className="text-center font-display text-2xl font-bold leading-tight text-ink lg:text-3xl">
          {teaser.title}
        </h3>
        <p className="mt-2 text-lg font-light leading-relaxed text-center text-muted">{teaser.text}</p>
        ...
        {/* The stage takes the height left under the text; the blob is the
            ...
            the flyer. Centred, so it stays under the centred text even if a
        ...
        <div className="event-stage relative mt-6 min-h-0 flex-1">
          <DancerBlob
            ...
            areaRef={ref}
            className="absolute bottom-0 left-1/2 w-[min(100cqw,110cqh)] -translate-x-1/2"
          />
        </div>
      </div>
    </div>
